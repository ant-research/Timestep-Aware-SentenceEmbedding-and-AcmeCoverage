...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        query = self.linear_query(query).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)
        key = self.linear_key(key).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)
        value = self.linear_value(value).view(batch_size, -1, self.h, self.d_k).transpose(1, 2)
...
class Encoder(nn.Module):
    ...
    def forward(self, word_embed, mask):
        ...
        for layer in self.layers:
            x = layer(x, mask)
        return self.norm(x * mask.unsqueeze(-1))
